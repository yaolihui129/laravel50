...
//		if(nowTime < startLine || nowTime > deadLine){
    ...
    $(".rank").on("click", function () {
        ...
        addroute(route1);
        //获取英雄榜数据(默认周排行)
		//$(".week_rank").trigger("click");
		//$(".total_rank").trigger("click");
        getHeroWeekData();
            ...
            success: function (backData) {
                ...
                if (backData.flag == "0") {
                    ...
                    if (!usefulData.bestDuration || !usefulData.bestPercent) {
						...
						bestScore=usefulData.bestScore;
                        $(".best_scores span i").eq(0).html(bestScore);
...
//第二页签---提升历程
        ...
        function getHeroWeekData_new() {
            ...
            //获取次数趋势分析数据
			var memId = memInfo.memberId;
            var url = basePath + "exam/numberofday?memberId=" + memId;
            $.ajax({
                type: 'GET',
                async: true,
                url: url,
            ...
            //获取分数趋势分析数据
			var memId = memInfo.memberId;
            var url_store = basePath + "exam/promote?memberId="+memId;
            $.ajax({
                type: 'GET',
                async: true,
                url: url_store,
                ...
                success: function (backData) {
                    ...
                    if (backData.flag == "0") {
                        ...
                        var option_store = {
                            ...
                            series: [{
                                name: '考试分数', //系列名称，用于tooltip的显示，legend 的图例筛选，在 setOption 更新数据和配置项时用于指定对应的系列
                                type: 'line',
                                smooth: true, //是否平滑曲线显示
                                symbol: 'circle', //标记的图形。ECharts 提供的标记类型包括 'circle', 'rect', 'roundRect', 'triangle', 'diamond', 'pin', 'arrow'
                                symbolSize: 5, //标记的大小，可以设置成诸如 10 这样单一的数字，也可以用数组分开表示宽和高，例如 [20, 10] 表示标记宽为20，高为10[ default: 4 ]
			...
			var memIdPro = memInfo.memberId;
            var product_url = basePath + "exam/getExamProduct?memberId="+memIdPro;
            $.ajax({
                type: 'GET',
                async: true,
                url: product_url,
        ...
        function getHeroList_new(top) {
			var memId = memInfo.memberId;
            var url = basePath + "exam/getAllExamLog?memberId="+memId;
            $.ajax({
                type: 'GET',
                async: true,
                url: url,
                                    ...
                                    success: function (backData) {
                                        ...
                                        if (backData.flag == "0") {
                                            ...
                                            layer.open({
                                                ...
                                                area: ['70%'],
                                                content: '<table border="" cellspacing="1" cellpadding="8" style="width: 100%;border:solid">' +
                                                '<tbody id="mm" >' +
                                                '<tr style="width: 100%;">' +
