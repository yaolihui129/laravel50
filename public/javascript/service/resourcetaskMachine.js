AutoTaskUtil = function(me) {
	...
	var taskUrl = "/resource/web/machine";
	...
	return me = {
			...
			function yes(requestData, callbackfn) {
				if (requestData) {
                    $.ajax({
                        type : 'GET',
                        data : requestData,
                        url : '/resource/web/machine/checkAdd',
		...
		del : function(ids) {
            $.ajax({
                ...
                url : '/resource/web/machine/checkDel',
                success : function(ret) {
                    var res=$.parseJSON(ret);
                    if(res.success==1){
		...
		edit : function(id) {
            $.ajax({
                ...
                url : '/resource/web/machine/checkEdit',
                success : function(ret) {
                    var res=$.parseJSON(ret);
                    if(res.success==1){
                            ...
                            .requestService(taskUrl + "/" + id + "/edit", "", false,
                                "get", function(response, status) {
                                    if (response.success) {
                                        ...
                                        $("#hostIp").val(task.hostIp);
                                        $("#vmName").val(task.vmName);
