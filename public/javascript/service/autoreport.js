		...
		reportLog : function(logs) {
			...
			for ( var idx in logs) {
				...
				if (log.scriptName
					...
					errorScirptHtml = scriptHtml = "<li data-toggle='collapse' data-parent='#accordion' href='.mm' class='o' style='color:#32b5cb;'><span></span>脚本："
							+ scriptName +
						"<a class='caret' style='float: right;margin-top: 14px' >" +
						"</a>" +
						"</li>";
				} else
					scriptHtml = "";
				...
				if ($.inArray(log.chrResult, fails) >= 0) {
					if($.inArray(log.chrResult, fail) >= 0){
                        $("#step_fail").append(
