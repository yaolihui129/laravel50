AutoTaskUtil = function (me) {
    ...
    var taskUrl = "/app/web/task";
    var execUrl = taskUrl + "/exec";
    var schemeUrl = "/app/web/scheme";
    ...
    return me = {
        ...
        openLayer: function (yes) {
            ...
            layer.open({
                ...
                yes: function (index, layero) {
                    ...
                    var row = $(".rowid").attr("id");
                    var row1 = row.split("_");
                    var id = row1[1];
                    if (!error) {
                        var requestData = {
            ...
            switch (opt) {
                case 0:// 选中
                    if (ifckeck) {
                        if (!projectId || projectId == undefined
                            ...
                            if (idx < 0) {
                                ...
                                $("#checkedScheme").append(
                                    "<span chk='shceme' id='" + conId + "' class='rowid'>"
                        ...
                        layer.msg("App任务选择应用必须唯一,请取消多余选项", {
                            offset: "50px"
                        });
                        if (idx >= 0) {
        ...
        report: function (id, row) {
            var browserId=row.browserId;
            var taskExecID=row.taskExecID;
            var browserName=row.browserNames;
            //var reportid=taskExecID+"-"+browserId;
            //alert("/log/"+reportid+".log");
            //location.href="/appreport"+"/"+taskExecID+"/"+browserId+"/"+browserName;
            ...
            layer.open({
                ...
                offset : [ '50px' ],
                area : [ "900px", "500px" ], // 宽高
                content:"/appreport"+'/'+taskExecID+'/'+browserId+'/'+browserName,
                success:function () {
                    $.ajax({
                        url:"/appreport"+'/'+taskExecID+'/'+browserId+'/'+browserName
                    })
                },
            });
