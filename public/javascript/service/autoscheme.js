	...
	return me = {
		initOwn : function() {
			$("#search").on(
					"click",
					function() {
                        var execBeginDate=$("#execBeginDate").val();
                        var execEndDate=$("#execEndDate").val();
                        var execTimebegin=$("#execTimebegin").val();
                        var execTimeend=$("#execTimeend").val();
                        if(execBeginDate=='' || execTimebegin==''||execEndDate=='' || execTimeend=='' ){
                            var beginTime='';
                            var endTime='';
						}else{
                            beginTime=execBeginDate+" "+execTimebegin;
                            endTime=execEndDate+" "+execTimeend;
						}
						var searchReqData = {
							"search" : {
            ...
            $("#execDateTimeBegin .input-daterange").datepicker({
                keyboardNavigation : false,
                forceParse : false,
                autoclose : true
            });
            $("#execDateTimeEnd .input-daterange").datepicker({
                keyboardNavigation : false,
                forceParse : false,
                autoclose : true
            });
            ...
            var month=chick(myDate.getMonth()+1);
            var date=chick(myDate.getDate());
            var myTime=year+"-"+month+"-"+date;
            $("#execBeginDate").val(myTime);
            $("#execEndDate").val(myTime);
		},
		init : function() {
			me.initOwn();
			me.createList();
