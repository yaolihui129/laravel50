...
class TimerTaskService
{
    ...
    public function getTimerTaskList($secho, $iDisplayStart, $iDisplayLength, $user, $search, $wl)
    {
        ...
        $rows = DB::select("select atit.id,atit.chrTiTaskName tiTaskName,dic.chrDictValue taskType,u.chrUserName userName,ate.chrBrowserNames browserNames ,arep.created_at createTime ,
				...
				INNER JOIN auto_projects apro on apro.id=atit.intProjectID 
				INNER JOIN users u on u.id=atit.intCreaterID
				LEFT JOIN (select DISTINCT intTimerTaskID,chrBrowserNames ,created_at 
