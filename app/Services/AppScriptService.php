...
class AppScriptService {
	...
	public function getScriptByParent($parentId, $user) {
		return DB::select ( "SELECT id,intProjectID as pId,chrScriptName as name,'false' isParent,'testcase' as iconSkin
				FROM auto_scripts where intProjectID=$parentId and intCompanyID=$user->intCompanyID and intType=1 order by id desc" );
	}
	/**
	 * 根据父节点 获取脚本
	 *
...
//				where aus.intFlag=0 and aus.intCompanyID=$user->intCompanyID and $search $wl  order by aus.id desc" );
        //4-20修改  增加aus.intTopProjectID=336的条件限制
        $res = DB::select ( "SELECT count(*) as allCount
				FROM auto_scripts aus
				inner join auto_projects aup on aup.id=aus.intProjectID
				inner join auto_projects apro on apro.id=aus.intTopProjectID
				inner join users u on u.id=aus.intCreaterID
				left JOIN (select DISTINCT intScriptID from auto_script_files)asf on asf.intScriptID=aus.id  
				where aus.intFlag=0 and aus.intCompanyID=$user->intCompanyID and aus.intTopProjectID='454' and $search $wl  order by aus.id desc" );
	...
	public function getScriptJsonByParent($secho, $iDisplayStart, $iDisplayLength, $user, $search, $wl) {
		...
		$rows = DB::select ( "SELECT aus.id,chrScriptName as scriptName,aup.chrProjectName as productName,apro.chrProjectName projectName ,asr.intAttID attId,
				...
				left JOIN (select DISTINCT intScriptID from auto_script_files)asf on asf.intScriptID=aus.id  
				where aus.intFlag=0 and aus.intCompanyID=$user->intCompanyID and $search and apro.intType=1 order by aus.id desc limit ?,?", [ 
				$iDisplayStart,
				$iDisplayLength 
		]);
	...
	//app信息完善新增service
    public function choose($scriptid,$name,$package_name,$version,$type,$classification,$remarks,$log,$description,$time){
        $query=DB::insert("INSERT into app_information(scriptid,name,package_name,version,type,classification,remarks,log,description,time) VALUES('$scriptid','$name','$package_name','$version','$type','$classification','$remarks','$log','$description','$time')");
        return $query;
    }
    //$scriptid是否存在校验service
    public  function check($scriptid){
        $query=DB::select("select scriptid  from app_information where scriptid=$scriptid");
        return $query;
    }
    //app信息完善更新service
    public function appupdate($scriptid,$name,$package_name,$version,$type,$classification,$remarks,$log,$description,$time){
        $query=DB::update("update app_information set name='$name',package_name='$package_name',version='$version',type='$type',classification='$classification',remarks='$remarks',log='$log',description='$description',time='$time' where scriptid='$scriptid'");
        return $query;
    }

//按scriptid获取路径及apk文件
//    //select appi.scriptid,appi.log,sysa.chrFile from
