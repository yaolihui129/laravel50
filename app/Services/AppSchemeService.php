...
//				where aus.intFlag=0 and aus.intCompanyID=$user->intCompanyID and $search $wl" );
        //4-20修改了SQL
        $res = DB::select ( "select COUNT(*) allcount
				...
				where aus.intFlag=0 and aus.intCompanyID=$user->intCompanyID and aus.intTopProjectID=454 and $search  order by aus.id desc" );
...
//		$rows = DB::select ( "select appi.scriptid id,
//appi.name schemeName,
//autop.chrProjectName projectName,
//us.chrUserName createUser,
//ate.id taskExecID,
//ate.chrBrowserNames browserNames,
//appi.version state
//from app_information appi
//join  auto_scripts autos ON autos.id=appi.scriptid
//join  auto_projects autop ON autop.id=autos.intProjectID
//join users us ON us.id=autop.intCreaterID
//left join auto_tasks ats ON ats.intCreaterID=us.id
//left JOIN auto_task_execs ate on ate.intTaskID=ats.id and ate.intFlag=0
//LEFT JOIN auto_projects apro on apro.id=ats.intProjectID" );
        //4-21重新修改了SQL
        $rows = DB::select ( "SELECT aus.id id,
aus.chrScriptName schemeName ,apro.chrProjectName projectName,u.chrUserName createUser
from auto_scripts aus 
JOIN auto_projects apro ON aus.intProjectID=apro.id
JOIN users u ON apro.intCreaterID=u.id
WHERE aus.intTopProjectID=454 and aus.intCompanyID=$user->intCompanyID and $search order by aus.id desc limit?,? ", [$iDisplayStart, $iDisplayLength]  );
...
//
//select appi.scriptid id, appi.name schemeName,autop.chrProjectName chrUserName,us.chrUserName createUser,ate.id taskExecID,ate.chrBrowserNames browserNames from
//app_information appi
//join  auto_scripts autos ON autos.id=appi.scriptid
//join  auto_projects autop ON autop.id=autos.intProjectID
//join users us ON us.id=autop.intCreaterID
