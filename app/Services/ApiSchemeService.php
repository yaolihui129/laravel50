...
class ApiSchemeService {
    ...
    private function getSchemeCount($user, $search, $wl) {
        $res = DB::select ( "select COUNT(*) allcount from auto_schemes aus
				...
				where aus.intFlag=0 AND apro.id in (502,503,505,506,507,508,509,510,533,541,660,661) and  aus.intCompanyID=$user->intCompanyID and $search $wl" );
    ...
    public function getSchemeList($secho, $iDisplayStart, $iDisplayLength, $user, $search, $wl) {
        ...
        $rows = DB::select ( "select DISTINCT aus.id,chrSchemeName schemeName,u.chrUserName createUser,ate.id taskExecID,ate.chrBrowserNames browserNames,ate.created_at createTime,
				...
				LEFT JOIN auto_task_execs ate on ate.intTaskID=ats.id and ate.intFlag=0 and ate.intTimerTaskID=0 and ate.intCreaterID=$user->id
				LEFT JOIN auto_projects apro on apro.id=ats.intProjectID
				where aus.intFlag=0 and aus.intCompanyID=$user->intCompanyID  and ats.intProjectID in (502,503,505,506,507,508,509,510,533,541,660,661) and $search $wl order by aus.id desc limit ?,?", [
            $iDisplayStart,
            $iDisplayLength
        ] );
    ...
    public function makeSchemePackage($id, $selBrowsers, $allBrowsers, $user) {
        ...
        if (! empty ( $endNodes )) {
            ...
            $resource = $root . DIRECTORY_SEPARATOR . "easyTestApi";
...
//                        // 写入xml文件
//                        $paramPath=StringUtil::iconv_UTF (  ( $paths ["proDir"] ) . $row->sfileName );
//                        $childNode = XMLHelper::createChildNode ( $doc, $newNode, "testCase" );
//                        XMLHelper::setNodeAttr ( $childNode, "paramPath", $paramPath );
...
//                        break;
                        ...
                        $paths = $this->copyScriptFiles ( $resource, $dest, $scriptFile, $sfileName, $scriptParam, $spfileName, $scriptId );
                        // 写入xml文件
                        $childNode = XMLHelper::createChildNode ( $doc, $newNode, "testCase", StringUtil::iconv_UTF ( " " ));
                        XMLHelper::setNodeAttr ( $childNode, "paramPath", StringUtil::iconv_UTF ( $paths ["proDir"] ) . $row->sfileName  );
                        XMLHelper::setNodeAttr ( $childNode, "enabled", "True" );
                        XMLHelper::setNodeAttr ( $childNode, "scriptId", $scriptId ); // 脚本ID
                        break;
                    }
        ...
        FileHelper::writefile_fopen ( $runfile, "Main('$schemeName.xml','INTERFACE').run()", "a" );
    }
    ...
    public function copyScriptFiles($resource, $dest, $scriptFile, $sfileName, $scriptParam, $spfileName, $scriptId, $copy = false) {
        $replace_path = "/database/schemes/easyTestApi";
        $testCaseDir = "/script/data/";
        $dataDir = "/script/data/";
