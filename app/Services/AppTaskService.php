...
class AppTaskService {
	...
	private function getTaskCount($user, $search, $wl) {
		$rows = DB::select ( "select COUNT(*) allCount 
from auto_tasks att
...
where ats.intTopProjectID='454' and $search ");
	...
	private function structureSearchSQL($search) {
		if (! empty ( $search )) {
			...
			if ($projectId) {
				array_push ( $sql, "ats.intTopProjectID=$projectId" );
			}
			if ($state === "") {
			...
			if ($taskName)
				array_push ( $sql, "att.chrTaskName='$taskName'" );
			if ($creater)
				array_push ( $sql, "us.chrUserName='$creater'" );
			$sql = implode ( " and ", $sql );
		}
		if (empty ( $sql ))
			$sql = "1=1";
