...
//                // 放入sysjobs中
//                $sysJService = new ApiJobsService();
//                $payload = array (
//                    'taskId' => $taskId,
...
//                    ] );
//                }
             DB::update ( "update auto_task_execs set intState=2 where intTaskID='$taskId'");
            }
            DB::commit ();
        } catch ( \Exception $e ) {
            DB::rollback ();
    ...
    private function createExecTask($taskId) {
        ...
        foreach ( $rows as $row ) {
            ...
            FileHelper::writefile_fopen ( $newname, "Main('$schemeName.xml','INTERFACE').run()" . "\r\n", "a" );
            Log::info("start");
        }
    ...
    public function copyScriptFiles($resource, $dest, $scriptFile, $sfileName, $scriptParam, $spfileName, $scriptId, $copy = false) {
        ...
        if (strrpos ( $destScriptDir, "worm_" ) === false)
            $proDir = substr($destScriptDir,42);
        else
            $proDir = $pathinfo ["dirname"] . "/";
        $destCaseDir = $dest . $dataDir . $proDir;
        // 参数
        ...
        if ($copy) {
            try{
                ...
                //if (! empty ( $sfileName ))
                    // 拷贝具体的脚本
                Log::info( "123");
                    copy ( $resource . $dataDir . $proDir . StringUtil::iconv ( $sfileName ), $destCaseDir . StringUtil::iconv ( $sfileName ) );
                //if (! empty ( $spfileName ))
                    // 拷贝具体的参数化文件
                Log::info( "456");
                    //copy ( $resource . $dataDir . $proDir . StringUtil::iconv ( $spfileName ), $destDataDir . StringUtil::iconv ( $spfileName ) );
                Log::info( "789");
            }catch (\Exception $ex){
                Log::info( "110");
                Log::info("脚本：".$resource . $dataDir . $proDir . StringUtil::iconv ( $sfileName )."\r\n");
                Log::info( "111");
