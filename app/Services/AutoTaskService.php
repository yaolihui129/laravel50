...
class AutoTaskService {
	...
	private function getTaskCount($user, $search, $wl) {
		$rows = DB::select ( "select COUNT(*) allCount from auto_tasks ats
				...
				where ats.intFlag=0 and apro.id not in (454,455,456,502,503,505,506,507,508,509,510,533,541,660) and ats.intCompanyID=$user->intCompanyID and intTaskType=0 and $search $wl" );
	...
	private function structureSearchSQL($search) {
		if (! empty ( $search )) {
			...
			if ($taskName)
				array_push ( $sql, "ats.chrTaskName like '%$taskName%'" );
			if ($creater)
	...
	public function getTaskList($secho, $iDisplayStart, $iDisplayLength, $user, $search, $wl) {
		...
		$rows = DB::select ( "select ats.id,chrTaskName taskName,u.chrUserName createUser,ate.chrBrowserNames browserNames ,ate.created_at createTime,
				...
				INNER JOIN auto_projects apro on apro.id=ats.intProjectID
				where ats.intFlag=0 and ats.intCompanyID=$user->intCompanyID and ats.intProjectID not in (454,455,456,502,503,505,506,507,508,509,510,533,541,660) and ats.intTaskType=0 and $search $wl ORDER BY ats.id desc  
				limit ?,?", [ 
				$iDisplayStart,
				$iDisplayLength 
		] );
