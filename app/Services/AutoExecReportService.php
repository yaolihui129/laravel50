...
class AutoExecReportService {
	...
	private function getReportCount($user, $search,$wl) {
		$res = DB::select ( "SELECT count(*) allCount from 
				...
				where  apro.id NOT in(339,340,341,342,343,344,345,346,348,349) and $search $wl" );
	...
	public function getReportList($secho, $iDisplayStart, $iDisplayLength, $user, $search, $wl) {
		...
		$rows = DB::select ( "SELECT a.id,case when a.intReportType=0 then '案例运行' when a.intReportType=1 then '普通任务' else '定时任务' end reportType,
				...
				INNER JOIN auto_projects apro on apro.id=a.intProjectID 
				INNER JOIN users u on u.id=a.intCreaterID 
				where  apro.id NOT in(339,340,341,342,343,344,345,346,348,349) and $search $wl ORDER BY id desc
				limit ?,?", [ 
				$pagecount,
				$iDisplayLength 
		] );
    ...
    public function getReportLogs($intScriptID,$intSchemeID,$intExecTaskID){
        $result = DB::select("SELECT l.intScriptID,s.chrScriptName,l.intLineNo,l.chrCmd,l.chrCmdParam,l.chrDescription,l.chrErrorMessage,l.chrResult,l.fltDuring from auto_logs l 
			LEFT JOIN auto_scripts s on l.intScriptID=s.id
			where l.intExecTaskID=$intExecTaskID  and  l.intSchemeID=$intSchemeID  and  l.intScriptID=$intScriptID ");
        return $result;
    }
