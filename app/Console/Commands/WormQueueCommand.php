		...
		try{
			...
			if ($daemon) { // 持续监听
				...
				while ( true ) {
					...
					if (empty ( $jobs )) {
						//echo date ( "Y-m-d H:i", time () ) . ":runWormer --no job sleep\r\n";
						sleep ( $sleep );
					} else {
						//echo date ( "Y-m-d H:i", time () ) . ":runWormer --dispachAutoTask \r\n";
						$ret = $wqService->dispachAutoTask ( $jobs, $connection, $queue, $delay, $memory, $daemon, $tries );
						//echo "test1";---------------------------------------------
						//echo date ( "Y-m-d H:i", time () ) . ":runWormer --Service-ret".$ret." \r\n";
			...
			echo date ( "Y-m-d H:i", time () ) . ":runWormer --error\r\n";
