<?php
namespace App\Http\Controllers\Resource\Web;
...
use App\Services\ResourceMachineService;
use App\Services\SysDictService;
use Illuminate\Http\Request;

class ResourceMachineController extends Controller {
	...
	{
		//返回机器管理首页
        return view ( "resource.web.resourcetaskMachine" );
	}
    ...
    public function getTaskList(Request $request) {
        ...
        $user = $request->user ();
        $atService = new ResourceMachineService();
    ...
    public function update(Request $request, $id) {
        ...
        $vmName=$request->input('vmName');
        $userName=$request->input('userName');
        $password=$request->input('password');
        $ateService = new ResourceMachineService ();
        $rows = $ateService->getTaskExecState ( $id,$taskName,$MacIP,$belongTo );
        if ($rows) {
            $user = $request->user ();
            $ateService->update ( $id, $task, $user,$taskName,$MacIP,$belongTo,$hostIp,$vmName,$userName,$password );
        } else{
	...
	public function checkAdd(Request $request)
    {
        ...
        $MacIP=$request->input('MacIP');
        $belongTo=$request->input('belongTo');
        $query=new ResourceMachineService();
