...
//                $filename = date('Y-m-d-H-i-s') . '-' . uniqid() . '.' . $ext;
                ...
                //中文处理的文件名
                $newName = iconv ( 'UTF-8', 'gbk', $originalName );
                //按名称复制到指定目录(中文/英文都复制,已防报错)
                copy(base_path()."/database/schemes/easyTest/SRC/app/".$originalName,base_path()."/database/schemes/easyTest/SRC/param/".$newName);
                //删除app下临时文件
                unlink(base_path()."/database/schemes/easyTest/SRC/app/".$originalName);

				$sucess = base_path()."/database/schemes/easyTest/SRC/param/".$newName;
                if(file_exists($sucess)){
					echo "<div class='text-center'>文件上传成功</div>";
				}else{
					echo "<div class='text-center'>文件上传失败</div>";
					}
            }
            exit;
        }
    ...
    //上传文件到C盘的方法
    public function uploadCfiles(Request $request)
    {
        if ($request->isMethod('POST')) {
            $file = $request->file('file');
            if ($file->isValid()) {
                ...
                $realPath = $file->getRealPath();
//                $filename = date('Y-m-d-H-i-s') . '-' . uniqid() . '.' . $ext;
                $bool = Storage::disk('uploadC')->put($originalName, file_get_contents($realPath));
                //中文处理的文件名
                $newName = iconv ( 'UTF-8', 'gbk', $originalName );
                //按名称复制到指定目录(中文/英文都复制,已防报错)
                copy("C:/app/".$originalName,"C:/data/".$newName);
				
                //删除app下临时文件
                unlink("C:/app/".$originalName);

                $sucess = "C:/data/".$newName;
				if(file_exists($sucess)){
					echo "<div class='text-center'>文件上传成功</div>";
				}else{
					echo "<div class='text-center'>文件上传失败</div>";
